...
def test_detect_rebound_shapes():
    price = np.array([1, 2, 3])
    indicator_values = np.array([1, 2, 3, 4])
    with pytest.raises(ValueError, match="price and indicator_values must have the same shape."):
        detect_rebound(price, indicator_values)


def test_detect_rebound_tolerance():
    price = np.array([1, 2, 3])
    indicator_values = np.array([1, 2, 3])
    with pytest.raises(ValueError, match="tolerance must be a non-negative number."):
        detect_rebound(price, indicator_values, tolerance=-1)


def test_detect_rebound_values():
    price = np.array([1, 2, 3, 2, 1])
    indicator_values = np.array([1, 2, 3, 2, 1])
    indices, rebound = detect_rebound(price, indicator_values)
    assert np.array_equal(indices, np.array([2]))
    assert np.array_equal(rebound, np.array([False, False, True, False, False]))


def test_detect_rebound_no_values():
    price = np.array([1, 2, 3, 4, 5])
    indicator_values = np.array([1, 2, 3, 2, 1])
    indices, rebound = detect_rebound(price, indicator_values)
    assert np.array_equal(indices, np.array([]))
    assert np.array_equal(rebound, np.array([False, False, False, False, False]))


def test_detect_rebound_nan_handling():  # To test the division by zero handling
    zero_indicator = np.array([0, 2, 3, 2, 1])
    price = np.array([1, 2, 3, 4, 5])
    indices, rebound = detect_rebound(price, zero_indicator)
    assert np.array_equal(indices, np.array([]))
    assert np.array_equal(rebound, np.array([False, False, False, False, False]))
